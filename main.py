...
def train_val(**train_val_arg_dict):
    unifiedmodel_obj, optimizer, train_dataloader, val_dataloader, n_epochs, print_every, experiment_dir, loss_, bce_with_logits_loss, device, use_lr_scheduler, trainable_weight2 = train_val_arg_dict.values()
    prev_time = time.time()
...
if __name__=='__main__':
    ...
    parser.add_argument('--exp_dir_with_captions', type=str,help='existing experiment_dir having train_val captions')
    ...
    exp_dir_with_captions = os.path.join(runs_dir,args.exp_dir_with_captions)
    # makedir(runs_dir)
    ...
    train_encoded_videos = pickle.load(open(os.path.join(exp_dir_with_captions,'train_encoded_video.pkl'),'rb'))
    val_encoded_videos = pickle.load(open(os.path.join(exp_dir_with_captions,'val_encoded_video.pkl'),'rb'))
    if exp_dir_with_captions:
    ...
    train_dataloader, val_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, collate_fn=collate_fn, num_workers=10),\
    DataLoader(VideoClipDataset(**val_dataset_dict), shuffle=False, batch_size=batch_size, pin_memory=True,  collate_fn=collate_fn, num_workers=10)
