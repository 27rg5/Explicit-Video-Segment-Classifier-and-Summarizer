...
def inference_on_val(videos_pkl, eval_dataset_type, checkpoint_path, root_dir_path, EncodeVideo_obj, modalities=['video','audio','text'], vanilla_fusion=False, pairwise_attention_modalities=False, experiment_name=None, get_classified_list=None, language_model_name='distilbert-base-uncased', video_model_name='slowfast_r50', spectrogram_model_name='resnet18', device='cuda:0', all_captions_dict=None, mlp_object=None, weighted_loss_mlp_fusion=None, run_caption_model=False):
    ...
    if run_caption_model:
        ...
        csv_writer.writerow(['Video path','Caption'])

    softmax_values, argmax_values = [], []
    start_time = time.time()
    for i, modality_inputs in tqdm(enumerate(val_dataloader)):
        with torch.no_grad():
            video_paths, transformed_video, processed_speech, spectrogram, caption, target = modality_inputs
            #pdb.set_trace()
            if run_caption_model:
                for video in video_paths:
                    summarized_string = summarize(video, model, processor, device)      
                    csv_writer.writerow([video, summarized_string])
...
if __name__=='__main__':
    ...
    inference_on_val(videos_pkl, eval_dataset_type, checkpoint_path, root_dir_path, EncodeVideo_obj, modalities, vanilla_fusion, pairwise_attention_modalities,device='cuda:1', get_classified_list=get_classified_list, experiment_name=experiment_name, run_caption_model=run_caption_model, mlp_object=mlp_object, weighted_loss_mlp_fusion=weighted_loss_mlp_fusion, all_captions_dict=all_captions_dict)
